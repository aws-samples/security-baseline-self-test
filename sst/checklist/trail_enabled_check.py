...
def check_trail_enabled(session) -> common.CheckResult:

    ret = common.CheckResult()

    ret.title = "Cloudtrail의 Trail 설정 확인"
    ...
    try:
        ...
        logging.error(traceback.format_exc())
        ret.level = level.error
        ret.msg = "관리형 계정의 Service Linked Role 이 존재하지 않습니다."
        ret.result_rows.append(["ERR", "ERR"])
    ...
    if len(trails) == 0:
        ...
        if logging_disabled_counter == len(trails):
            ret.level = level.danger
            ret.msg = "모든 Trail의 Logging 설정이 비활성화 되어있습니다. Logging 을 활성화 해주세요."
        elif logging_disabled_counter > 0 and logging_disabled_counter < len(trails):
            ret.level = level.warning
            ret.msg = "일부 Trail의 Logging 설정이 비활성화 되어있습니다. Logging 을 활성화 해주세요."
        else:
            ret.level = level.success
            ret.msg = "모든 Trail 의 Logging 설정이 활성화 되어있습니다."
